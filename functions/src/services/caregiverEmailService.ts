...
async function generateVisitPDF(data: VisitPDFData): Promise<Buffer> {
  ...
  if (data.diagnoses.length > 0) {
    ...
    content.push({ ul: data.diagnoses.map(d => formatDiagnosis(d)), margin: [0, 5, 0, 15] });
  }
  ...
  if (data.nextSteps.length > 0) {
    ...
    content.push({ ul: data.nextSteps.map(s => formatNextStep(s)), margin: [0, 5, 0, 15] });
  }
...
function formatMed(med: unknown): string {
  ...
  if (typeof med === 'object' && med !== null) {
    const m = med as { name?: string; dose?: string; dosage?: string; display?: string; frequency?: string };
    if (m.display) return m.display;
    let result = m.name || 'Unknown';
    if (m.dose || m.dosage) result += ` (${m.dose || m.dosage})`;
    return result;
  }
  return 'Unknown';
}
...
/**
 * Format a diagnosis item (may be string or object)
 */
function formatDiagnosis(diag: unknown): string {
  if (typeof diag === 'string') return diag;
  if (typeof diag === 'object' && diag !== null) {
    const d = diag as { name?: string; text?: string; condition?: string; display?: string };
    return d.display || d.name || d.text || d.condition || 'Unknown condition';
  }
  return 'Unknown condition';
}

/**
 * Format a next step / action item (may be string or object)
 */
function formatNextStep(step: unknown): string {
  if (typeof step === 'string') return step;
  if (typeof step === 'object' && step !== null) {
    const s = step as { text?: string; description?: string; title?: string; action?: string; display?: string };
    return s.display || s.text || s.description || s.title || s.action || 'Action item';
  }
  return 'Action item';
}
...
export async function sendVisitPdfToAllCaregivers(
  ...
  let activeCaregivers = caregivers;
              ...
              <table width="100%">
                <tr>
                  <td style="color: #475569; font-size: 14px;">
                    <strong>${data.patientName}</strong> • ${data.visitDate}
                  </td>
          ...
          <tr>
            <td style="padding: 40px;">
              ...
              <ul style="color: #475569; font-size: 15px; line-height: 1.6; margin: 0; padding-left: 20px;">
                ${data.diagnoses.map(d => `<li>${formatDiagnosis(d)}</li>`).join('')}
              </ul>
              ` : ''}
              ...
              <ul style="color: #475569; font-size: 15px; line-height: 1.6; margin: 0; padding-left: 20px;">
                ${data.nextSteps.map(s => `<li>${formatNextStep(s)}</li>`).join('')}
              </ul>
              ` : ''}
