import { FirestoreActionRepository } from '../FirestoreActionRepository';
...
describe('FirestoreActionRepository', () => {
  it('adds soft-delete defaults when creating actions', async () => {
    const harness = buildCreateHarness();
    const repository = new FirestoreActionRepository(harness.db);
  ...
  it('filters deleted records by default when listing paginated actions', async () => {
